...
from src.db import models, config
...
class PostgresPipeline(object):
    ...
    def process_item(self, item: dict, spider):
        ...
        for author_s in item["authors"]:
            author = models.Author(name=author_s)  # type: ignore
            self.session.add(author)
            self.session.commit()
            self.session.add(models.BookAuthor(book_id=book.id, author_id=author.id))  # type: ignore
            self.session.commit()
